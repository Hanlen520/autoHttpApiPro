...
class ExcelReport():
    ...
    def init(self, worksheet, data):
        ...
        worksheet.merge_range('A1:F1', '接口测试报告总概况', define_format_H1)
        worksheet.merge_range('A2:F2', '测试概括', define_format_H2)
        worksheet.merge_range('A3:A6', '优胜教育', get_format_center(self.wd))
        # worksheet.insert_image('A3:A6', 'logo.png')
        _write_center(worksheet, "B3", '项目名称', self.wd)
        _write_center(worksheet, "B4", '接口版本', self.wd)
        _write_center(worksheet, "B5", '脚本语言', self.wd)
        _write_center(worksheet, "B6", '测试环境', self.wd)
        ...
        _write_center(worksheet, "D6", "执行时长", self.wd)
        ...
        _write_center(worksheet, "F3", "健康状态分数", self.wd)
        if int(data['test_sum'])==0:
            scroe=100
        else:
            scroe=round((int(data['test_success'])/int(data['test_sum']))*100,2)
        worksheet.merge_range('F4:F6', scroe, get_format_center(self.wd))
